...
fs.mkdirSync('out/revision-v2/stills',{recursive:true});
const serveUrl=await bundle({entryPoint:path.resolve('src/index.ts')});
const browser=await openBrowser('chrome');const snapshots=[];
try{
 const inputProps={qa:true},composition=await selectComposition({serveUrl,id:DATA.id,inputProps,puppeteerInstance:browser});
 const frames=[...new Set([0,8,18,25,75,150,230,340,374,...DATA.scenes.flatMap(s=>[s.from,s.from+14,s.to-1]),...DATA.captions.map(p=>p.from+2)])].sort((a,b)=>a-b);
 for(const frame of frames){let result;
 await renderStill({serveUrl,composition,inputProps,frame,puppeteerInstance:browser,output:`out/revision-v2/stills/f${frame}.png`,onBrowserLog:log=>{if(log.text.startsWith('__EDUCA_QA__'))result=JSON.parse(log.text.slice('__EDUCA_QA__'.length));}});
...
fs.writeFileSync('out/revision-v2/qa.json',JSON.stringify({snapshots},null,2));
